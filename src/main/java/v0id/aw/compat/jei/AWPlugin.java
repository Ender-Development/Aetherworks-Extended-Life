...
public class AWPlugin implements IModPlugin
{
    ...
    public void registerCategories(IRecipeCategoryRegistration registry)
    {
        registry.addRecipeCategories(
                new MetalFormerCategory(registry.getJeiHelpers().getGuiHelper()),
                new AnvilCategory(registry.getJeiHelpers().getGuiHelper()));
    }

    @Override
    public void register(IModRegistry iModRegistry)
    {
        iModRegistry.addRecipes(MetalFormerRecipes.recipes.stream().map(MetalFormerWrapper::new).collect(Collectors.toList()), AWConsts.catIDMetalFormer);
        iModRegistry.addRecipeCatalyst(new ItemStack(AWBlocks.FORGE_COMPONENT, 1, 4), AWConsts.catIDMetalFormer);
        iModRegistry.addRecipes(AetheriumAnvilRecipes.recipes.stream().map(AnvilWrapper::new).collect(Collectors.toList()), AWConsts.catIDAnvil);
